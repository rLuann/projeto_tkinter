...
def iniciar_figura_nova(event): 
    global figura_nova
    cor_borda = cor_borda_var.get()  # Pega a cor selecionada
    ...
    if tipo_figura_var.get() == 'Linha':
        ...
        figura_nova = ("circulo", (event.x, event.y, event.x, event.y), cor_borda)
    elif tipo_figura_var.get() == 'Retângulo':
        figura_nova = ("retangulo", (event.x, event.y, event.x, event.y), cor_borda)
...
def desenhar_figuras():
    canvas.delete("all")
    for fig, values, cor_borda in figuras:
        if fig == "linha":
            canvas.create_line(values[0], values[1], values[2], values[3], fill=cor_borda)
        elif fig == "rabisco":
            canvas.create_line(values, fill=cor_borda)
        elif fig == "circulo":
            canvas.create_oval(values[0], values[1], values[2], values[3], outline=cor_borda)
        elif fig == "retangulo":
            canvas.create_rectangle(values[0], values[1], values[2], values[3], outline=cor_borda)

def desenhar_figura_nova():
    fig, values, cor_borda = figura_nova
    if fig == "linha":
        ...
        canvas.create_oval(values[0], values[1], values[2], values[3], outline=cor_borda, dash=(4, 2))
    elif fig == "retangulo":
        canvas.create_rectangle(values[0], values[1], values[2], values[3], outline=cor_borda, dash=(4, 2))

def incompleta(figura):
    fig, values, cor_borda = figura
    if fig == "linha":
...
frame_cor = Frame(frame)
frame_cor.grid(column=0, row=1, sticky=W, **paddings)

cor_borda_var = StringVar(root, value="black")

ttk.Label(frame_cor, text="Cor da Borda:").grid(column=0, row=0, padx=5)
borda_combo = ttk.Combobox(frame_cor, textvariable=cor_borda_var,
